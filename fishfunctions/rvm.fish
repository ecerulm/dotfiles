function rvm --description='Ruby enVironment Manager'
  ...
  env PATH="" /bin/bash -lc 'source ~/.rvm/scripts/rvm; rvm "$@" ; status=$?; env > "$0"; exit $status' $env_file $argv

  # apply rvm_* and *PATH variables from the captured environment
  eval (grep '^rvm\|^[^=]*PATH\|^GEM_HOME' $env_file | grep -v '_clr=' |grep -v rvm_debug| sed '/^[^=]*PATH/s/:/" "/g; s/^/set -xg /; s/=/ "/; s/$/" ;/; s/(//; s/)//')
  # clean up
  rm -f $env_file
end
