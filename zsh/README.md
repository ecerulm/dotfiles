...
| `rlm-git-status [-a\|--all] [-b\|--base <ref>] [<directory>]` | `git-status` | Status roll-up across every git repo in a directory — written for the multi-repo collection dirs created by `rlm-pr-worktree`'s multi-repo mode. Scans the immediate children of `<directory>` (default cwd) that are their own git worktree root; when the target is itself a repo with no repo children it reports on just that repo. Each repo gets two independent columns: `VS <base>` commit divergence (`+N` ahead, `-N` behind, `=` in sync, `?` when the base ref is missing) and `LOCAL` uncommitted work (conflict/staged/unstaged/untracked counts, or `clean`). Only repos in either state are listed; the rest collapse onto one dim `unchanged:` line with a shared boilerplate prefix (`data-platform-`) stripped when it is long enough to be worth removing. Base defaults to `origin/<default>` (from `origin/HEAD`, falling back to `origin/main`); nothing is fetched, so divergence reflects the remote-tracking refs as they stand locally. Repos are probed in parallel. Colors only when stdout is a tty, so the table stays pasteable. Requires: `git`. |
