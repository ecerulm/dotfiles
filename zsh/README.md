...
| `rlm-git-squash-branch` | `git-squash-branch` | Squash every commit on the current branch since it diverged from origin's default branch into one signed-off commit. Uses `git symbolic-ref --short refs/remotes/origin/HEAD` for the default branch and `git merge-base --fork-point` (with plain `git merge-base` fallback) for the base. Runs `git reset --soft <base>` then `git commit -s`, and prints the `git push --force-with-lease` command needed to publish. Refuses on detached HEAD, on the default branch, or when nothing is left to squash. Requires: `git`. |
