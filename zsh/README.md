...
| `rlm-dbt-sandbox` | `dbt-sandbox` | Create personal dbt sandbox BigQuery datasets in `$DBT_DEV_PROJECT`. For each layer L in `$DBT_SANDBOX_LAYERS` (colon-separated, e.g. `prep:psa:dw:rd`), creates dataset `<DBT_DATASET_PREFIX>_<L>` with label `sandbox:true` and a 30-day default table expiration. Idempotent — skips datasets that already exist. Refuses to run if `$DBT_DEV_PROJECT` ends in `-prod`. Configure all three env vars in `~/.zshrc.thismachine`. Requires `bq`. |
