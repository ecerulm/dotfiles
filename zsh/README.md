...
| `rlm-gar-open` | `gar-open` | Browse all container images across one or more GCP projects via fzf and open the selected image in the Artifact Registry console. Projects are read from `$GAR_SEARCH_PROJECTS` (colon-separated, shared with `gar-search`); Artifact Registry locations from `$GAR_SEARCH_LOCATIONS` (default `europe:europe-west1:us`) — repos are listed per location, so a location missing there is invisible. Image list is cached in `~/.cache/gar-images/`; selecting `--- REFRESH CACHE ---` opens a secondary fzf picker (with an `*** ALL PROJECTS ***` sentinel) where you TAB-multi-select which projects to re-fetch — entries for un-picked projects are preserved. Picker is MRU-sorted. Prints the fully qualified image path and the GCP console URL, copies the URL to the clipboard, and opens it in the browser. GCR layer-cache pseudo-packages (`<image>/cache`) are filtered out. Requires `gcloud`, `fzf`, `jq`, `curl`, `python3`. |
| `rlm-gar-rm` | `gar-rm` | Multi-select Artifact Registry `<repo>/<package>` entries via fzf and permanently delete them. Shares the package-list cache and MRU history with `gar-open` (`~/.cache/gar-open/`), including the `--- REFRESH CACHE ---` partial-refresh picker. The preview is deletion-oriented: version count, total size, newest/oldest version dates, tag count, package create/update time and annotations, and the number of 1000-version delete batches required; an auth or permission failure is reported loudly rather than rendering as an empty package. Deletes in two phases — versions in batches of 1000 via `gcloud artifacts versions delete --delete-tags` (the server rejects a single `packages delete` above 1000 versions), then the empty package — printing the exact gcloud command lines before the per-package confirmation. Re-queries after deleting to verify, offers a retry when versions remain, and aborts a package whose batch made no progress. Successfully deleted entries are pruned from the cache and history. `-n`/`--dry-run` shows everything and runs nothing. Requires `gcloud`, `fzf`, `jq`, `curl`, `python3`. |
| `rlm-gar-version-rm` | `gar-version-rm` | Delete individual **versions** inside a single package (where `gar-rm` deletes whole packages). Two pickers: pick one `<repo>/<package>` using the shared gar-\* package picker, then TAB-multi-select versions inside it. Versions list newest-first with date, size, short digest and tags (untagged dimmed); the preview shows the image name, every tag on the digest, the full SHA, size, media type (flagging multi-arch indexes), upload/update/create/build times, platform and other scalar metadata, and warns when a digest carries several tags since deleting it removes them all. The version list is fetched once and handed to the preview as a cached file, so scrolling costs no API calls. Prints the exact gcloud commands, confirms once, deletes in batches of 1000, then re-lists and verifies each selected digest is gone, offering a retry for any that remain. Selecting every version leaves an empty package behind — flagged before confirming, with a pointer to `gar-rm`. `-n`/`--dry-run` shows everything and runs nothing. Requires `gcloud`, `fzf`, `jq`, `curl`, `python3`. |
