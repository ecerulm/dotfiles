...
| `rlm-pr-worktree-rm-merged-closed [--dry-run\|-n] [--yes\|-y]` | `pr-worktree-rm-merged-closed` | Clean up stale worktrees via a single fzf picker. Mode auto-detected: inside a git repo → that repo only; otherwise scans the cwd for primary git repos (child dirs with a `.git` directory) and gathers worktrees across all of them. One picker lists every worktree in scope (excluding each repo's main/current; the dir-name prefix identifies the repo), flagged by category — `[merged#N]`/`[closed#N]`/`[no-pr/no-diff]` (deletable, sorted first), `[open#N]`, `[has-work]`; TAB multi-selects, ENTER deletes (y/N confirm), Ctrl-G aborts. Afterwards prints a combined table of the REMAINING worktrees with staleness: `REPO WORKTREE BRANCH BEHIND AHEAD AGE CREATED` (behind/ahead vs the default branch, AGE = last-commit date, CREATED = dir birthtime; main shown as `(main)`). `--dry-run` skips the picker and just prints the deletable set + table; `--yes` is accepted but ignored. Same `worktree remove` → `worktree remove --force` fallback as `pr-worktree-rm`. |
...
| `rlm-fe` | `fe` | Fuzzy-pick one or more files reachable from the current directory (recursive; includes hidden and gitignored files, excludes `.git/`) via fzf and open them in `$EDITOR`. Tab marks files for multi-select; preview shows file contents. Entries are sorted by recently-selected first, then by file mtime descending. The MRU cache stores full absolute paths (`~/.cache/rlm-fe/`), so a file keeps its rank regardless of which directory `fe` is run from. Requires: `fd`, `fzf`, `python3`. |
