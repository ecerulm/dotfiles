...
- `_rlm-jira-cache` — fetch / read the shared JIRA-issue cache at `~/.cache/rlm-jira/items.tsv` (one issue per line, tab-delimited raw fields: `<group>\t<KEY>\t<status>\t<summary>\t<assignee>`, where group is `1=assigned`, `2=reporter`, `3=watching`, `4=DATA-only` — within each group rows are kept in JQL `ORDER BY updated DESC`, and across groups KEYs are deduped in priority order so an "assigned" hit always wins over a "DATA-only" hit for the same ticket). Subcommands: `path` (emit `cache_path=`, `history_path=` lines for `eval` — keys are `cache_path` / `history_path`, NOT bare `cache` / `history`, because `history` is a read-only variable in zsh and callers `eval`-ing this output would otherwise fail with "read-only variable: history"), `refresh` (run the four JQL queries in parallel via `acli jira workitem search`, dedupe across groups, atomically rewrite `items.tsv`; freshness gate is `(resolved is EMPTY OR resolved >= -60d)` so tickets closed > ~2 months ago drop off; per-group `--limit 70` so the cache is bounded at 4 × 70 = 280 rows before cross-group dedupe — small enough that the `column -t | fzf` picker pipe never gets near the SIGPIPE-into-pipefail silent-exit class of bugs, but still wide enough to cover the active working set on each group), `read` (`cat` the cache file), `age` (humanized age of the cache mtime — `23s ago` / `4m ago` / `2h ago` / `3d ago`). Used by `rlm-jira-pick` and `rlm-pr-worktree`. Originally inline in `rlm-jira-pick` (`_jira_pick_fetch` + `_jira_pick_cache_age`); lifted into a shared helper so `rlm-pr-worktree` reads the same cache without round-tripping to JIRA on every pick. The raw, uncolored cache row format (no ANSI, no rendered display column) lets each caller render its own display column on top — `rlm-jira-pick` adds the colored 4-group label + `BOLD` KEY + `GRAY` status; `rlm-pr-worktree` drops the group column and reshapes to `JIRA\t<KEY>\t[<status>]\t<summary>\t[<assignee>]` for its `column -t` alignment alongside PR rows.
- `_rlm-jira-history` — read/write the shared JIRA MRU history at `~/.cache/rlm-jira/history.txt` (one KEY per line, oldest first / newest at the bottom, capped at 1000 lines). Subcommands: `path` (print the absolute path), `append <KEY>` (append + re-cap via the `tail -1000 | print >|` dance), `read` (stream the file as-is — caller `tac`/`tail -r` for MRU order), `migrate-legacy` (one-shot idempotent copy of `~/.cache/jira-pick/history.txt` to the new path if the new file doesn't exist yet — legacy file is not deleted, same convention as `_rlm-bq-history` and `_rlm-dbt-history`). Used by `rlm-jira-pick` (which calls `migrate-legacy` at the top, then `append` after every successful pick) and `rlm-pr-worktree` (which calls `append` only after a successful `_prwt_make_jira_worktree`, so a cancelled worktree-name prompt doesn't pollute the MRU). The shared history means a pick made through either function floats the same ticket to the top of the other's picker on the next call.
- `_rlm-jira-sorted-rows` — emit cache rows from `_rlm-jira-cache` in MRU-then-cache order: history entries (newest first, deduped) whose KEY still exists in the cache, then the remaining cache rows in their stored group/recency order. Takes no arguments; reads cache and history paths from the two helpers above (so a single source of truth governs where those files live). Stdout: one raw tab-delimited cache row per line (`<group>\t<KEY>\t<status>\t<summary>\t<assignee>`) in final sort order; empty stdout if no cache exists (caller is expected to call `_rlm-jira-cache refresh` in that case). Used by `rlm-jira-pick` and `rlm-pr-worktree`. Originally inline in `rlm-jira-pick` (`_jira_pick_sorted_list`); lifted so both callers share the same MRU-first ordering. The grep anchor for "is this KEY in the cache?" is tab-on-both-sides (`grep -F -- "\t${key}\t"`), so KEYs that are substring prefixes of other KEYs (e.g. `DATA-1` vs `DATA-12`) don't false-match the wrong row.
