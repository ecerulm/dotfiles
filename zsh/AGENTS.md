...
- `_rlm-python-ensure-version` — verifies that pyenv's `python` shim resolves an interpreter from the given cwd, by running `python -EsSc 'import sys; print(sys.executable)'` (the exact probe Poetry uses internally when bootstrapping a venv). Signature: `(cwd, label)`. On non-zero exit prints a multi-line diagnostic (failing command, cwd, captured stderr/stdout, and a hint pointing at `.python-version` / `pyenv install`) and returns 1. Used by `_rlm-dbt-cmd`, `rlm-dbt-ls`, and `rlm-dbt` (the generic dispatcher), gated on `${DBT_USE_FUSION:-0} != 1` (the Fusion path runs `dbtf`, a standalone binary that does not touch pyenv). Always runs *before* `_rlm-poetry-ensure-venv` in callers that use both — pyenv-shim failures otherwise surface several layers down as a misleading "poetry install failed" (poetry shells out to `python -EsSc ...` to discover the interpreter, and pyenv's helpful "no such version" message gets buried in `subprocess.CalledProcessError`). The cwd matters because pyenv only walks up from the cwd to find `.python-version` — in layouts where `pyproject.toml` lives in a parent dir and `.python-version` lives in a child (e.g. work repo's `data-platform-dbt-.../storytel-dbt/.python-version`), callers must pass the child dir (`dbt_project_root` for `rlm-dbt-*`), not the pyproject.toml parent. Skipped silently in `rlm-dbt` when no `--project-dir` is resolvable (user didn't pass one and walk-up found nothing) so dbt itself emits its own "no dbt_project.yml" error.
- `_rlm-dbt-bin` — emit the command-prefix tokens (one per line) for invoking dbt across every `rlm-dbt-*` command. Two modes: when `DBT_USE_FUSION=1`, emits a single line containing the absolute path the `dbtf` alias resolves to (typically `$HOME/.local/bin/dbt`); otherwise emits the three lines `poetry`, `run`, `dbt` (project-pinned dbt-core, the historical default). Callers consume the output by splitting on newlines: `local -a dbt_bin; dbt_bin=(${(f)"$(_rlm-dbt-bin)"}) || return 1; "${dbt_bin[@]}" ls --select tag:finance`. Critically, Fusion mode emits the **resolved path** rather than the alias name `dbtf`, because every caller is an `emulate -L zsh` function (which sets `NO_ALIASES` locally) AND invokes the array as `"${dbt_bin[@]}"` (quoted array expansion never triggers alias expansion); either condition alone would turn a bare `dbtf` into `command not found` — silently, because callers redirect stderr to `/dev/null`. Fusion mode resolves the alias by reading zsh's alias table directly via `${aliases[dbtf]}` (works regardless of the local `NO_ALIASES` setting — `command -v dbtf` does not), validates the path is an executable file, validates `<path> --version 2>&1` mentions "fusion" (case-insensitive `grep -i fusion`), and caches the resolved path in a `typeset -g _RLM_DBT_BIN_FUSION_PATH` shell-global so subsequent calls in the same shell skip the alias lookup AND the `--version` fork-exec. Non-Fusion mode does no validation (that's `_rlm-poetry-ensure-venv`'s job for the caller). Return codes: 0 on success; 1 when `DBT_USE_FUSION=1` but the `dbtf` alias is undefined / resolves to a missing file / its `--version` output does not look like Fusion (diagnostic to stderr; negative results are not cached so a fix-then-retry within the same shell works). Centralizes the fusion/poetry switch so a future rename of the alias or a stricter validation rule has exactly one site to edit instead of one per `rlm-dbt-*` command.
...
- `_rlm-dbt-nodes-cache` — fetch / read the shared per-project nodes cache at `~/.cache/rlm-dbt/<project_key>/nodes.json` (one JSON object per line, from `dbt ls --output json` covering every dbt resource_type: model + seed + test + snapshot + source + analysis + exposure). Subcommands: `path <project_key>` (cache file path), `refresh <project_key> <project_root> <dbt_project_root> <profiles_dir> <target> [<profile>]` (re-fetch and atomically rewrite the cache), and `read <project_key> <jq_filter>` (apply a `jq -r` filter and print one entry per line). Lets each `rlm-dbt-*` command filter the shared cache by `resource_type` (`select(.resource_type == "model") | .name` for run/ls, `or .resource_type == "seed"` for build, `"test"` for test, `rlm-dbt-find-source-code` reads every type and renders `<resource_type>:<short_name>` entries) instead of fetching its own `dbt ls`. Used by `_rlm-dbt-cmd`, `rlm-dbt-ls`, and `rlm-dbt-find-source-code`. The on-disk JSON also feeds `dbt-model-preview` in rlm-dbt-ls's Phase 1 preview pane. The `refresh` subcommand prints the full `poetry run dbt ls …` command in `$ (cd <project_root> && <cmd>)` form before running it, and validates the first non-empty line of output parses as JSON before overwriting the cache — dbt sometimes emits compilation errors (`Encountered an error: …`) to stdout with a non-zero exit code the grep pipeline masks; without the validation guard the cache would be silently overwritten with the error message and `read` would return zero matches.
...
- `_rlm-bq-refresh-sentinels` — shared `--- REFRESH CACHE ---` / `--- REFRESH SANDBOX CACHE ---` sentinel logic for every `rlm-bq-*` / `rlm-sandbox-bq-*` fzf picker. Subcommands: `emit-main [--tab]` and `emit-sandbox [--tab] [--no-color]` (print the sentinel string for the picker input — `--tab` prefixes a literal tab for `--with-nth=2` layouts, `--no-color` drops the PURPLE/RESET wrap on the sandbox sentinel so sandbox-only pickers like `rlm-sandbox-bq-{open,rm}` don't render their own scope as differently-colored noise); `is-main <stripped-ref>` and `is-sandbox <stripped-ref>` (return 0 when the ANSI-stripped ref matches the corresponding sentinel — callers feed in `$sel_ref`, not `$sel_line`, so the helper does not need to know which layout the caller used); `apply-main` (delete both `$cache` and `$sandbox` then `_rlm-bq-cache refresh`) and `apply-sandbox` (delete `$sandbox` only then `_rlm-bq-cache refresh --sandbox-only`). The historical inline `_bq_cache_age` formatter — verbatim across all 5 callers — now lives inside this helper as `_rlm_bq_cache_age` (one extra detail: renders `"unknown"` when the file does not exist, instead of leaking a negative-`elapsed` string into the picker). Cache file paths come from `_rlm-bq-cache paths` so callers don't have to thread `$cache` / `$sandbox` through. Used by `rlm-bq-open`, `rlm-bq-archive`, `rlm-bq-rm-tables`, `rlm-sandbox-bq-open`, `rlm-sandbox-bq-rm`. Adding a 6th caller — or changing the sentinel wording, or adding a freshness threshold to the age formatter — is now a single-site edit. Note: the `apply-main` action also deletes the sandbox cache (matches what the original inline `rlm-bq-open` code did on the theory that a user asking for a full refresh wants both halves rebuilt); if a future caller needs main-only deletion semantics, that's an opt-in flag, not a default.
- `_rlm-jira-cache` — fetch / read the shared JIRA-issue cache at `~/.cache/rlm-jira/items.tsv` (one issue per line, tab-delimited raw 7-field rows: `<group>\t<KEY>\t<status>\t<summary>\t<assignee>\t<updated_epoch>\t<last_commented_epoch>`, where group is `1=assigned` / `2=reporter` / `3=watching` / `4=DATA-only`, deduped across groups in priority order, and the two epoch fields are integer Unix seconds — `updated_epoch` from `fields.updated`, `last_commented_epoch` = `max(fields.updated || created)` across `fields.comment.comments[]` or `0` when an issue has no comments). Subcommands: `path` (emit `cache_path=`, `history_path=` lines for `eval` — keys are `cache_path` / `history_path`, NOT bare `cache` / `history`, because `history` is a read-only variable in zsh and callers `eval`-ing this output would otherwise fail with "read-only variable: history"), `refresh` (two phases: **phase A** = four parallel `acli jira workitem search` bulk queries with the freshness gate `(resolved is EMPTY OR resolved >= -60d)` and per-group `--limit 20` so up to 4 × 20 = 80 raw rows; **phase B** = per-issue parallel `acli jira workitem view <KEY> --fields key,updated,comment --json` via `xargs -P20` because acli rejects `updated` and `comment` in the bulk-search `--fields` flag with "Error: fields 'updated, comment' are not allowed", each worker writes its raw JSON to `$tmp_views_dir/<KEY>.json`, then a single Python pass parses all of them and merges with phase A's 5-field rows into the final 7-field items.tsv — measured ~5–8 seconds for 50–80 issues against the real JIRA instance), `read` (`cat` the cache file), `age` (humanized age of the cache mtime — `23s ago` / `4m ago` / `2h ago` / `3d ago`). Used by `rlm-jira-pick` and `rlm-pr-worktree`. Originally inline in `rlm-jira-pick` (`_jira_pick_fetch` + `_jira_pick_cache_age`); lifted into a shared helper so `rlm-pr-worktree` reads the same cache without round-tripping to JIRA on every pick. The two trailing epoch fields exist for `_rlm-jira-sorted-rows`'s composite-key sort (`max(picked_at, updated_epoch, last_commented_epoch)`); each caller drops them in its own renderer — `rlm-jira-pick`'s inline python3 renderer reads parts[0..3] for the colored display column, `rlm-pr-worktree`'s `awk 'NF>=7 {print "JIRA", $2, "[" $3 "]", $4, "[" $5 "]"}'` reshape emits fields 1–5 in the format `column -t` expects.
- `_rlm-jira-history` — read/write the shared JIRA MRU history at `~/.cache/rlm-jira/history.txt` (TSV: `<epoch>\t<KEY>`, one entry per line, oldest first / newest at the bottom, capped at 1000 lines, duplicate KEYs across appends tolerated and resolved as max-per-KEY by the sort consumer). Subcommands: `path` (print the absolute path), `append <KEY>` (write `<date +%s>\t<KEY>` + re-cap via the `tail -1000 | print >|` dance — also runs the legacy-format auto-migration if needed before appending so the file always settles into TSV), `read` (auto-migrate then stream the file as-is — single caller is `_rlm-jira-sorted-rows`'s composite-key sort), `migrate-legacy` (one-shot idempotent copy of `~/.cache/jira-pick/history.txt` to the new path if the new file does not yet exist, then convert single-column → TSV — legacy file is not deleted, same convention as `_rlm-bq-history` / `_rlm-dbt-history`). Legacy single-column format is auto-upgraded in place via `_rlm_jira_history_autoupgrade`: detect with `grep -q $'\t'`, then awk-rewrite assigning synthetic timestamps `now - rank * 60` where rank=0 is the bottom (newest) line — preserves the existing MRU order exactly, atomic `mv` from a temp file. Used by `rlm-jira-pick` (which calls `migrate-legacy` at the top, then `append` after every successful pick) and `rlm-pr-worktree` (which calls `append` only after a successful `_prwt_make_jira_worktree`, so a cancelled worktree-name prompt doesn't pollute the MRU). The shared history means a pick made through either function influences the sort order of the other's picker on the next call.
- `_rlm-git-changed-history` — shared MRU helper for `rlm-git-changed` (and any future `rlm-git-*` command that wants to share the MRU). Modeled on `_rlm-dbt-history`: one pooled TSV at `~/.cache/rlm-git-changed/history.tsv` with columns `<epoch>\t<command>\t<repo_key>\t<path>`, scoped per repo via `repo_key` = md5 of the absolute `git_root` (computed inline in `rlm-git-changed`, symmetrical to `_rlm-dbt-project-key`). Deduped on the `(repo_key, path)` tuple — re-picking the same file overwrites its prior entry so the newest pick wins. Capped at 500 lines, atomic tmp+mv on every write. Subcommands: `append <command> <repo_key> <path>` and `read <repo_key>` (the latter filters to the requested repo, dedupes on path, emits `<epoch>\t<command>\t<path>` newest-first). Used only by `rlm-git-changed` today. Legacy migration: on first read/append after the new TSV does not exist, `_rlm_git_changed_history_legacy_migrate` folds in entries from every `~/.cache/git-changed/<md5>.history` legacy file under the md5 in the filename as the `repo_key` (so per-repo buckets survive the migration even though no shell context knows which `git_root` produced them), assigning synthetic timestamps `now - rank * 60` with rank=0 for the newest line so the existing MRU order survives exactly. Legacy files are not deleted, matching the `_rlm-bq-history` / `_rlm-dbt-history` convention. Note: the `rlm-git-changed` caller assembles the four-line python payload (`git_root` + `__HISTORY__` + helper output + `__FILES__` + file list) inside a `{ … } | python3` brace-group — the original version (b56f466) emitted the prints to the function's stdout (fed to fzf) and piped only the file list into python3, causing `ValueError: list.index("__HISTORY__")` on every invocation.
- `_rlm-jira-sorted-rows` — emit cache rows from `_rlm-jira-cache` in **composite-key DESC order**: each row's sort_key is `max(picked_at, updated_epoch, last_commented_epoch)` where `picked_at` is the most-recent timestamp from `history.txt` for the row's KEY (or 0 if never picked), `updated_epoch` is field 6 of the cache row, and `last_commented_epoch` is field 7. A single global sort over every cache row — no history/non-history tier — so a freshly-commented ticket can outrank an old picked one, and a never-picked-but-recently-updated ticket can outrank a picked-but-stale one. The 4-group priority survives only as a tiebreaker for the (extremely rare) case where two rows share a sort_key (group_idx ASC, so assigned wins over DATA-only). Takes no arguments; reads cache + history paths from the two helpers above (so a single source of truth governs where those files live). Stdout: one raw 7-field cache row per line in final sort order; empty stdout if no cache exists (caller is expected to call `_rlm-jira-cache refresh`). Used by `rlm-jira-pick` and `rlm-pr-worktree`. Originally inline in `rlm-jira-pick` (`_jira_pick_sorted_list`, a hand-rolled "history rows first, then the rest" interleave); replaced with an inline Python composite-key sort because the dict-lookup + max + sort idiom is much cleaner in Python than awk + multi-stage shell. Calls `_rlm-jira-history read >/dev/null` once before sorting so a legacy single-column history is auto-migrated to TSV before the picked_at parser runs (otherwise every KEY's picked_at would silently fall back to 0 and the sort would degenerate to `max(updated, commented)`-only).
