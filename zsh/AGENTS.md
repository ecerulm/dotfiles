...
- **Locations**: repositories are listed **per location** — the API has no all-locations query (`locations/-` → `INVALID_ARGUMENT`), and `gcloud artifacts repositories list` without `--location` silently returns only its default location's repos. `$GAR_SEARCH_LOCATIONS` (colon-separated, default `europe:europe-west1`) is the scan list; a location missing from it is invisible to every gar-\* picker. Do not "fix" this by enumerating all ~46 GCP locations — that is 46 round-trips per project to find the 2-3 that hold anything.

- `_rlm-gar-cache` — shared package-list cache, `~/.cache/gar-open/<md5>.txt` (one `<updated_epoch>\tLOCATION-docker.pkg.dev/PROJECT/REPO/PACKAGE` per line) plus a shared `history.txt` MRU. Key is md5 of the sorted+deduped project list, matching the key `rlm-gar-open` computed inline before the extraction — the existing cache is reused, not forked. Actions: `paths <projects…>` (emit `cache_file=`/`history_file=` for `eval`), `refresh <label> <all_projects_csv> <projects…>` (re-fetch names via the `packages.list` REST endpoint plus one `versions.list` per package for the age, **merging** — entries for un-refreshed projects are preserved), `read [--by-age] <projects…>` (MRU-sorted by default; `--by-age` sorts oldest-push first and ignores history), `age <projects…>` (humanized mtime, empty when no cache), `remove <projects_csv> <image…>` (drop deleted packages — matches on the image field, not the whole line). Filters GCR `<image>/cache` layer-cache pseudo-packages.

- **Package age**: the sort key is the **newest version's** `updateTime`, fetched with one `versions.list` per package (`orderBy=update_time desc` + `pageSize=1`), fanned out 16-at-a-time. Do **not** revert this to the `package.updateTime` that `packages.list` returns for free — an earlier version of this file claimed the two were equal and told you not to query `versions`. That claim was **wrong**. `package.updateTime` also advances on metadata writes and registry-side housekeeping: measured over a 46-package cache, 2 disagreed, one by **379 days** (`eu.gcr.io/airbyte-custom-connectort/storytel-google-directory` reported `5d`, newest version `2025-07-24`). The error direction is what makes it disqualifying on a delete tool — a dead package looks fresh and sinks to the bottom of the `--by-age` list. The per-package cost is small and paid only on an explicit refresh: 46 packages in ~1s, ~0.17s per call.

- **Legacy cache rows**: caches written before the timestamp column hold a bare image path with no tab. Every reader tolerates that — caches are only rewritten on an explicit refresh, and forcing a re-fetch on upgrade would make the first gar-\* command after the change block on a full rescan. `read` normalizes untimestamped rows to epoch `0`, which sorts first under `--by-age` and renders as `unknown`: on a delete tool, an entry whose age is unknown should be looked at, not skipped.

- `_rlm-gar-pick-package` — the whole cache-backed package picker: initial fetch, cache-age label, `--- REFRESH CACHE ---` sentinel with the project sub-picker, ordered rows, standard fzf chrome. `([--by-age], label, prompt, preview_script, multi, projects…)`; emits selected image path(s). Rows are `<display>\t<image>` with `--with-nth=1` (never also `--nth` — see the fzf section), so the age column is display-only and callers always get bare paths; the preview receives `{2}`. `--by-age` (used by `rlm-gar-rm`) lists least-recently-pushed first with a relative-age column instead of MRU order — the stale packages are the point of a delete tool, and MRU buries them. Handles the refresh sentinel internally and reopens, so callers never see it and don't re-enter themselves. Deliberately does **not** record MRU — only the caller knows whether the selection was acted on (a delete the user then declines shouldn't reorder the picker).
...
- `_rlm-gar-index-children` — map every child digest of every multi-arch index in a package back to its parent. `(image, versions_json_file)`; emits `<child>\t<parent>\t<platform>\t<parent_tags_csv>`. **Why it must fetch manifests:** `docker images list` returns an index's per-arch members as ordinary untagged versions with nothing linking them to the parent, and the child manifest carries no `subject` and no annotations either — the relationship exists *only* inside the index body, so the only way to recover it is one registry GET per index. Measured on `eu.gcr.io/catalogue_export`: 44 of 46 untagged versions were members, 0 were true orphans. Fans out 16-at-a-time **inside a subshell** — `unsetopt monitor notify` alone does not stop `[8] - done curl …` in an interactive shell, because the jobs are reaped as options restore on return. A failed per-index fetch is skipped, not fatal: partial attribution beats none. `architecture: unknown` (buildx attestation/provenance manifests) collapses to the platform string `unknown`; those are members, not orphans.

- `_rlm-gar-version-cache` — per-package cache under `~/.cache/gar-version/<md5 of image>/`: `versions.json` (the listing, 10-minute TTL), `children.tsv` (attribution, never expired by time), `resolved.txt` (indexes already fetched). Actions `paths`/`age`/`load [--ttl-minutes N] [--force]`/`invalidate`/`forget <digest…>`. **The attribution never expires because a digest names an immutable manifest** — an index cannot change what it references without becoming a different digest — so `load` only fetches indexes new since last time, which is what makes thousands of versions viable (cold 205 versions/41 indexes ~3.1s, warm ~0.17s). A failed refresh falls back to the cached listing rather than reporting an empty package. `resolved.txt` rows are `<digest>` or `<digest>\tempty`: the **explicit** empty-marker is load-bearing — inferring "childless" as `resolved.txt` minus the parents seen in `children.tsv` makes the effective set equal `resolved.txt` by construction, so a pruned/truncated `children.tsv` is undetectable and its families are never re-fetched. Do not "simplify" it back. Both files are **pruned against the listing on every refresh**: rows naming versions that no longer exist are dropped (child rows need BOTH ends live — a row whose parent was deleted otherwise keeps rendering its child as part of a vanished family). `forget` alone is not enough, because versions also disappear without this tool — a repo cleanup policy deleting untagged versions on a timer left `ingress-dbt` with 24 of 25 `resolved.txt` rows naming deleted digests. Pruning runs only against a non-empty listing: an empty one means the fetch failed and would erase the cache.

- **There is no gcloud way to delete a subset of versions.** `gcloud artifacts versions delete` takes a **single** VERSION positional (GA, alpha and beta alike) and dies with `unrecognized arguments: sha256:…` when handed a list; `docker images delete` takes one image; there is no `batch-delete` subcommand and no generic REST caller. `rlm-gar-version-rm` therefore posts to `versions:batchDelete` via curl, chunked at **75** — the server-enforced cap, probed (76 → "A maximum of 75 versions are allowed per request"). This is the second time the single-positional trap has bitten: it made `rlm-gar-rm` a no-op first. Do not "simplify" the REST call back to a gcloud one-liner.

- **A 200 from batchDelete is not proof of deletion** — the API completes the operation with `error: null` for names that do not exist (probed with a bogus package). The post-delete re-list in Stage 4 is what actually establishes the outcome; never drop it in favour of the exit code.

- **Deletion order is parents-before-children**, enforced by depth-sorting the selected digests in `rlm-gar-version-rm` before batching. The registry refuses to delete a manifest a live index references. Do not "simplify" this away on the grounds that the picker already renders families parent-first — it does, but a **nested index** (index → index → member) breaks it: children are grouped by their immediate parent's timestamp, so an inner index sorts inside the outer family while its own members sort by the inner index's (possibly older) timestamp and render as an earlier family. `P3(Aug) -> P4(Jan) -> c4` lists `c4` first. Depth-sorting also makes the 1000-digest batch split irrelevant, since any prefix of a parents-first list is parents-first. Members whose index is *not* in the selection are a different problem that ordering cannot solve — those are listed before the confirmation instead.

- **Previews must not make network calls on the hot path.** An fzf preview re-runs on *every keystroke*, so anything per-call is multiplied by typing speed. `bin/gar-rm-preview` was ~2.8s per keystroke: ~1.4s `docker images list`, ~0.63s `gcloud auth print-access-token` (Python startup, not network), ~0.2-0.5s `packages.get`. Now ~0.49s, all local. Three caches, each with a different rationale:

  - **The version listing** is read from `~/.cache/gar-version/<md5>/versions.json` — the same file `_rlm-gar-version-cache` owns, verified field-identical to a live call. The preview also *writes* it when absent, which is what makes browsing the package picker usable; the picker then reads it (`from_cache=1`) instead of refetching. The write is only reachable when no cache existed, so a preview can never clobber a fresher picker listing. It never writes `children.tsv`/`resolved.txt` — index attribution is one registry GET per index and belongs to the picker.
  - **The access token** goes through `bin/gcloud-token-cached` (120s TTL, mode 600). Shared by all three gar-\* previews.
  - **Package metadata** (`packages.get`) is cached per package as `package.json`, 1h; only a response with `.name` is cached, so a transient 403 is not pinned.

  Call `_rlm-gar-version-cache load` from a preview and you undo all of this — `load` refetches past its TTL and resolves new indexes. Read the file directly and label its age instead.

- **A child can have MORE THAN ONE parent index** — the blocked-selection check must track the full parent set, not a `child -> parent` dict. Successive builds sharing an identical layer set produce distinct indexes referencing the same member digest (48 such children in the `ingress-dbt` cache). With a dict, a multi-parent child keeps only whichever parent was read last, so selecting the child plus that one parent reports "nothing blocked" and the registry then refuses the delete. The OCI rule is that an artifact is deletable only once *nothing* references it, so every referencing index outside the selection has to be named. This shipped broken once; do not "simplify" the `defaultdict(set)` back to a dict.

- **`validateOnly` is not a pre-flight for any of this.** `versions:batchDelete` accepts `names[]` and `validateOnly` only ([API reference](https://docs.cloud.google.com/artifact-registry/docs/reference/rest/v1/projects.locations.repositories.packages.versions/batchDelete)) — there is **no `force` field**, whatever a web search may claim. Probed against a known-refused index member, `validateOnly` returned **success**: it does not run reference checks. Every refusal warning is therefore computed locally from the child map and the repo config.

- **Tagged versions delete fine** via `batchDelete` (deleting by digest takes the tags with it) — probed. The `--delete-tags` flag belongs to `gcloud artifacts docker images delete` and covers deleting by *tag*, not this call. The exception is a repo with `dockerConfig.immutableTags`, where tags "cannot be modified, moved or deleted" and a tagged version is refused; `REMOTE`/`VIRTUAL` repos own nothing deletable at all. Both are repo-level, fetched once per package and reported before the confirmation.

- **`rlm-gar-version-rm` loops back to the package picker** after each package. The per-package flow lives in an inner `_gvrm_round` function so its ~15 `return` points still mean "this round is over"; the loop exits only on **3**, the code reserved for aborting the *package* picker. Plain `130` (version-picker abort, declined confirmation) deliberately reopens it. `--refresh` is cleared after the first round — left sticky it re-resolves every index of every later package.

- **Do not `rm` `$version_cache`/`$child_map` in `rlm-gar-version-rm`** — since the cache landed they are the persistent cache files, not mktemp copies. Two leftover `rm -f` lines from the pre-cache version silently deleted the cache on every run; the symptom is a picker that re-resolves every index each time while still *appearing* to work.
...
- **List navigation** — fzf's built-in `alt-<`/`alt->` (first/last) do **not** work here. `ghostty.conf` sets `macos-option-as-alt = left`, so right-Option sends no Alt: the terminal emits a bare `Escape` then the character, and fzf reads the lone `Escape` as **abort**. In a picker that loops (`rlm-gar-version-rm` reopens the package picker on 130) that silently discards the selection and jumps up a level rather than reporting an error. Bind `ctrl-t:first`/`ctrl-e:last` instead — same reasoning that makes `ctrl-g:abort` mandatory. `ctrl-e` costs fzf's default `end-of-line` on the query line, which is the right trade in a fuzzy-searched picker.
- **Select-all** — `ctrl-a:select-all`/`ctrl-d:deselect-all` (as in `rlm-afw-deploy`, `rlm-gar-version-rm`). Both act on the **currently matched** rows, not the whole list, so on a destructive picker an empty query selects everything — narrow with a query first.
...
- **`?` key overlay** — when a picker has more bindings than the header can hold, bind `?:preview(printf %s "$VAR")` to show the full list in the preview pane and keep the header to the few keys worth advertising. Restore the real preview by chaining `+refresh-preview` onto cursor movement (`up`/`down`/`ctrl-j`/`ctrl-k`) rather than relying on Esc, which is the key this repo already treats as unreliable. Hoist the preview command into a variable so the overlay and the normal preview cannot drift. Check first that `?` is not searchable content in that picker. Used by `rlm-gar-version-rm`.
...
For BigQuery, git-worktree, dbt-model, and file previews, call the shared scripts (full path) instead of inlining `bq show`/jq or JIRA/`gh`/`stat` logic:
...
- `"$HOME/bin/dbt-model-preview" <nodes_json> <dbt_project_root> {} [<prod_manifest>]` — name/unique_id/resource_type/package/materialized/tags/meta/description, upstream count + names, `bq_relation` (OSC 8 link to the BigQuery console, colored by project suffix), file path and mtime, then a **columns** section and the model's **raw source**. Used by both of `rlm-dbt-ls`'s pickers. Columns come from the prod manifest's `.columns`, so they need the optional `<prod_manifest>` arg and cover only what a `schema.yml` documents (409 of 1681 models in storytel-dbt) — the section is omitted, not left empty, for the rest. Source is the raw Jinja read from `original_file_path` on disk rather than the manifest's `compiled_code`: it is the file you would edit and it tracks the working tree. Rendered with `cat`, deliberately unhighlighted — `bat` is absent here and `pygmentize` is a pyenv shim whose Python startup would be paid on every keystroke.

All five are at `~/dotfiles/bin/` symlinked to `~/bin/`; being on `$PATH` is what makes them callable from the preview subshell.
